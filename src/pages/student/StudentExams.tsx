...
export default function StudentExams() {
  ...
  const className = (student?.class_id ? classMap[student.class_id] : '10') ?? '10';
  const section = (student?.section_id ? sectionMap[student.section_id] : 'A') ?? 'A';
      ...
      {upcoming.length > 0 && (
        <Card>
          ...
          <CardContent className="space-y-2">
            {upcoming.map(e => <ExamSummaryHeader key={e.id} exam={e} />)}
          </CardContent>
        </Card>
      )}
